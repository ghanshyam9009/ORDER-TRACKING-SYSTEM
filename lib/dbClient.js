...
async function closePosition(userId, posId, status, {
  ...
  try {
    ...
    if (item) {
      contributionAmount = item?.contributionAmount?.N ? parseFloat(item.contributionAmount.N) : 0;
      stopLoss = item?.sl?.N ? parseFloat(item.sl.N) : 0;
      takeProfit = item?.tp?.N ? parseFloat(item.tp.N) : 0;
      symbol = item?.symbol?.S || stockSymbol;
  ...
  const orderPayload = {
    ...
    stockSymbol: symbol ? { S: symbol } : { S: "UNKNOWN" },
  ...
  try {
    ...
    console.error(`❌ Failed to save order record for ${posId}:`, err);
