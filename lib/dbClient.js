...
async function fetchAllOpenPositions() {
  ...
  return data.Items.filter(
    ...
    return {
      ...
      contributionAmount:item.contributionAmount?.S || "0",
      status,
    };
  });
}
...
async function closePosition(
  ...
  const safeQuantity = parseFloat(quantity);
  const BROKERAGE_FEE = 0.23529412;
  const contributionAmounts = contributionAmount || 0;
  ...
  // 🔹 Pure PnL calculation (don't subtract brokerage)
  let realizedPnL =
    positionType === "LONG"
      ? (safeExitPrice - safeEntryPrice) * safeQuantity
      : (safeEntryPrice - safeExitPrice) * safeQuantity;
  ...
  // 4️⃣ Fund updates (Two-step: First PnL, then brokerage deduction)
  try {
    await updateUserFunds(userId, realizedPnL); // Step 1: Add PnL
    await updateUserFunds(userId, -BROKERAGE_FEE); // Step 2: Deduct brokerage
  } catch (err) {
