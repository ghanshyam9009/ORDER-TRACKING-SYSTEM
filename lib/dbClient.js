...
async function closePosition(
 ...
 const orderPayload = {
    ...
    createdAt: { S: closedAt },
    updatedAt: { S: closedAt },
    fee: toNumAttr(BROKERAGE_FEE.toFixed(8)),
    feeInINR: toNumAttr(feeInINR),
    leverage: toNumAttr(leverage),
    lot: toNumAttr(safeQuantity), // must be string unless using DocumentClient
    marginAmount: toNumAttr(contributionAmount),
    currency: { S: currency },
    status: { S: "FILLED" },
    orderType: { S: "MARKET" },
    operation: { S: positionType === "LONG" ? "SELL" : "BUY" },
    orderMessage: { S: "Order filled at market" },
    stockSymbol: { S: symbol },
    price: toNumAttr(safeExitPrice),
    priceInINR: toNumAttr(0),
    size: toNumAttr(safeQuantity),
    totalValue: toNumAttr(safeExitPrice * safeQuantity),
    stopLoss: toNumAttr(stopLoss),
    takeProfit: toNumAttr(takeProfit),
    positionID: { S: posId },
    metaData: {
      M: {
        source: { S: orderSource },
