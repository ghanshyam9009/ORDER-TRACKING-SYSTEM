...
async function fetchAllOpenPositions() {
  ...
  : item.initialQuantity?.N
    ...
    : 0,
        ...
        order: item.order?.M ? {
          operation: item.order.M.operation?.S,
          price: item.order.M.price?.N ? parseFloat(item.order.M.price.N) : undefined
        } : undefined,
        status
      };
    });
...
async function closePosition(userId, posId, status, {
  ...
  // ✅ Check values are defined and finite numbers
  ...
  // ✅ Fetch contributionAmount from DB
  ...
  // ✅ Add contribution to realized PnL
  ...
  const cmd = new UpdateItemCommand({
    ...
    UpdateExpression:
      'SET #status = :status, exitPrice = :exitPrice, closedAt = :closedAt, realizedPnL = :pnl',
    ExpressionAttributeNames: {
      '#status': 'status'
    },
    ExpressionAttributeValues: {
