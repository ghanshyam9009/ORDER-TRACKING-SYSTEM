...
//     // ⬇️ Fetch again from memory and apply updates now
//     const latestPos =
//       positions[symbol]?.[userId]?.[posId] ||
//       pendingPositions[symbol]?.[userId]?.[posId];

//     if (latestPos) {
//       Object.assign(latestPos, updates);

//       if (!latestPos.stopLoss && latestPos.sl) {
//         latestPos.stopLoss = { triggerPrice: { N: String(latestPos.sl) } };
//       }

//       if (!latestPos.takeProfit && latestPos.tp) {
//         latestPos.takeProfit = { triggerPrice: { N: String(latestPos.tp) } };
//       }

//       const derivedType = getDerivedPositionType(latestPos);
//       if (derivedType && latestPos.status === "OPEN") {
//         await addPosition(symbol, userId, {
//           ...latestPos,
//           type: derivedType,
//         });
//         console.log(
//           `✅ Live trackable position updated (after re-add): ${symbol} - ${userId} - ${posId}`
//         );
//       }
//     }
...
//   if (!derivedType) {
//     console.warn(
//       `⚠️ No SL/TP found. Skipping re-add for ${symbol} - ${userId} - ${posId}`
//     );
//     return;
//   }
...
//     console.log(
//       `✅ Live trackable position updated: ${symbol} - ${userId} - ${posId}`
//     );
//   }
// }
