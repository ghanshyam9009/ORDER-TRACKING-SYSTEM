...
function getDerivedPositionType(pos) {
  const sl = Number(
    pos?.stopLoss?.triggerPrice?.N ??
    pos?.sl
  );

  const tp = Number(
    pos?.takeProfit?.triggerPrice?.N ??
    pos?.tp
  );
...
async function updatePosition(symbol, userId, posId, updates) {
  ...
  if (!derivedType) {
    console.warn(`⚠️ No SL/TP found in position ${symbol} - ${userId} - ${posId}, skipping add`);
    return;
  }
    ...
    for (const posId in symbolUsers[userId]) {
      ...
      if (pos.type === 'sl' && markPrice <= pos.sl) {
        ...
        await closePosition(userId, posId, 'CLOSED', {
          exitPrice: pos.sl, // ← use trigger price
          triggerPrice: pos.sl,
