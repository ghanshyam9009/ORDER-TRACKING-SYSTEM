...
function getDerivedPositionType(pos) {
  const sl =
    pos?.stopLoss?.triggerPrice?.N || pos?.sl;
  const tp =
    pos?.takeProfit?.triggerPrice?.N || pos?.tp;

  if (sl && tp) return 'sltp';
  if (sl) return 'sl';
  if (tp) return 'tp';
  return null;
}
...
async function updatePosition(symbol, userId, posId, updates) {
  ...
  // 🛠️ Normalize SL/TP structure for derived type detection
  if (currentPos.sl && !currentPos.stopLoss) {
    currentPos.stopLoss = {
      triggerPrice: { N: String(currentPos.sl) }
    };
  }
  if (currentPos.tp && !currentPos.takeProfit) {
