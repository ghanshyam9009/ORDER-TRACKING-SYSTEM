...
const snsClient = new SNSClient({ region: "ap-southeast-1" }); // ✅ your AWS region
...
function getDerivedPositionType(pos) {
  ...
  const trig = Number(pos?.triggeredPrice);          // 👈 NEW

  console.log("📊 [getDerivedPositionType] Input:", { sl, tp, trig });

  const hasSL = !isNaN(sl) && sl > 0;
  const hasTP = !isNaN(tp) && tp > 0;
  const hasLimit = !isNaN(trig) && trig > 0;         // 👈 NEW

  if (hasSL && hasTP) return "sltp";
  if (hasSL) return "sl";
  if (hasTP) return "tp";
  if (hasLimit) return "limit";                      // 👈 NEW
  return null;
}
...
async function updatePosition(symbol, userId, posId, updates) {
  ...
  if (positions[symbol]?.[userId]?.[posId]) {
    ...
    console.warn(
      `⚠️ Position not found in memory: ${symbol} - ${userId} - ${posId}`
    );
    const found = await fetchPositionById(posId);

    if (!found) {
      console.error(
        `❌ Could not find position in DB: ${symbol} - ${userId} - ${posId}`
      );
      return;
    }
    ...
    console.log(
      `✅ Re-added from DB (with updates): ${symbol} - ${userId} - ${posId}`
    );
    return;
  }
  ...
  if (currentPos.type === "limit" && currentPos.triggeredPrice) {
    if (!currentPos.limit) {
      currentPos.limit = {
        triggerPrice: { N: String(currentPos.triggeredPrice) },
      };
  ...
  if (!derivedType) {
    console.warn(
      `⚠️ No SL/TP or TriggeredPrice found. Skipping re-add for ${symbol} - ${userId} - ${posId}`
    );
    return;
  }
  ...
  if (currentPos.status === "OPEN") {
    ...
    console.log(
      `✅ Position updated in memory: ${symbol} - ${userId} - ${posId}`
    );
  }
}
      ...
      for (const posId of posIds) {
        ...
        if (isValidEntry) {
          if (orderCategory === "LONG_LIMIT") {
            console.log(
              `📈 Tracking LONG_LIMITORDER: markPrice=${markPrice}, entryPrice=${entryPrice}`
            );
            if (markPrice <= entryPrice) {
              console.log(
                `✅ LONG_LIMITORDER HIT: markPrice (${markPrice}) >= entryPrice (${entryPrice})`
              );
              shouldOpen = true;
            }
          } else if (orderCategory === "SHORT_LIMIT") {
            console.log(
              `📉 Tracking SHORT_LIMITORDER: markPrice=${markPrice}, entryPrice=${entryPrice}`
            );
            if (markPrice >= entryPrice) {
              console.log(
                `✅ SHORT_LIMITORDER HIT: markPrice (${markPrice}) <= entryPrice (${entryPrice})`
              );
              shouldOpen = true;
            }
          }
        ...
        if (shouldOpen) {
          ...
          if (orderID) {
            if (isTrackable) {
              await markOrderAsOpened(orderID, entryPrice);
